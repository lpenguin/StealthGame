        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (DragAndDrop.objectReferences.Length > 0 && position.Contains(evt.mousePosition))
            {
                ...
                if (draggedObject == null ||
                    (draggedObject != null && draggedObject.GetComponent(constraint.Type) == null))
                {
                    DragAndDrop.visualMode = DragAndDropVisualMode.Rejected;

                    if (evt.type == EventType.DragExited)
                        Debug.LogError(string.Format("Object assigned to '{0}' must implement interface '{1}'",
                            property.name, constraint.Type));
                }
            }
